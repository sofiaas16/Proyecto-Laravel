const translations = {
    es: { title: "Vocabulario de Colores", subtitle: "Une cada color con su nombre", instructions: "Haz clic en un color y luego en su nombre correspondiente", colorsTitle: "Colores", namesTitle: "Nombres", successTitle: "¡Felicitaciones!", successMessage: "Has completado todos los pares correctamente", restartBtn: "Reiniciar", backBtn: "Volver", correctFeedback: "¡Correcto!", incorrectFeedback: "Inténtalo de nuevo", colors: { red: "Rojo", blue: "Azul", green: "Verde", yellow: "Amarillo", purple: "Morado", orange: "Naranja", }, },
    en: { title: "Color Vocabulary", subtitle: "Match each color with its name", instructions: "Click on a color and then on its corresponding name", colorsTitle: "Colors", namesTitle: "Names", successTitle: "Congratulations!", successMessage: "You have completed all pairs correctly", restartBtn: "Restart", backBtn: "Back", correctFeedback: "Correct!", incorrectFeedback: "Try again", colors: { red: "Red", blue: "Blue", green: "Green", yellow: "Yellow", purple: "Purple", orange: "Orange", }, },
    fr: { title: "Vocabulaire des Couleurs", subtitle: "Associez chaque couleur à son nom", instructions: "Cliquez sur une couleur puis sur son nom correspondant", colorsTitle: "Couleurs", namesTitle: "Noms", successTitle: "Félicitations!", successMessage: "Vous avez terminé toutes les paires correctement", restartBtn: "Recommencer", backBtn: "Retour", correctFeedback: "Correct!", incorrectFeedback: "Essayez encore", colors: { red: "Rouge", blue: "Bleu", green: "Vert", yellow: "Jaune", purple: "Violet", orange: "Orange", }, },
};
...
function createColorBoxes() {
    ...
    Object.keys(colorData).forEach((colorKey) => {
        ...
        colorBox.className = "color-box w-full h-24 rounded-xl cursor-pointer relative group";
        ...
        speakBtn.className = "absolute top-2 right-2 bg-white bg-opacity-80 hover:bg-opacity-100 rounded-full w-8 h-8 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100";
        speakBtn.innerHTML = '<i class="fas fa-volume-up text-gray-700 text-sm"></i>';
        speakBtn.onclick = (e) => { e.stopPropagation(); speakText(translations[currentLanguage].colors[colorKey]); };
...
function createNameCards() {
    ...
    shuffledKeys.forEach((colorKey) => {
        ...
        nameCard.className = "name-card bg-gray-50 hover:bg-gray-100 border-2 border-gray-200 rounded-xl p-4 cursor-pointer text-center relative group";
        ...
        const speakBtn = document.createElement("button");
        speakBtn.className = "absolute top-2 right-2 bg-blue-500 hover:bg-blue-600 text-white rounded-full w-8 h-8 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100";
...
function showFeedback(message, isCorrect) {
    ...
    content.innerHTML = `<i class="fas ${isCorrect ? "fa-check-circle text-green-500" : "fa-times-circle text-red-500"} text-4xl mb-2"></i>
                           <p class="text-xl font-semibold ${isCorrect ? "text-green-700" : "text-red-700"}">${message}</p>`;
...
function speakText(text) {
    if ("speechSynthesis" in window) {
        ...
        utterance.lang = currentLanguage === "es" ? "es-ES" : currentLanguage === "en" ? "en-US" : "fr-FR";
        utterance.rate = 0.8;
