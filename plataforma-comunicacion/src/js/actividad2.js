...
function makeMatch(colorKey) {
  ...
  const colorBox = document.querySelector(
    `.color-box[data-color="${colorKey}"]`
  );
  const nameCard = document.querySelector(
    `.name-card[data-color="${colorKey}"]`
  );
...
function completarActividad(activityNumber) {
  try {
    ...
    if (activityNumber === progreso + 1) {
      ...
      console.log(
        `Actividad ${activityNumber} completada. Progreso actualizado a: ${activityNumber}`
      );
