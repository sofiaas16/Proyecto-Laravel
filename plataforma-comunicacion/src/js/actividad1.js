...
const translations = {
  ...
  function changeLanguage(lang) {
    ...
    document.querySelectorAll(".language-btn").forEach((btn) => {
      if (btn.dataset.lang === lang) {
        btn.className =
          "language-btn bg-white text-blue-500 px-4 py-2 rounded-lg font-semibold text-lg hover:bg-blue-50 transition-colors"
      } else {
        btn.className =
          "language-btn bg-blue-400 text-white px-4 py-2 rounded-lg font-semibold text-lg hover:bg-blue-300 transition-colors"
      }
    })
  ...
  function completarActividad(activityNumber) {
    try {
      // Get current progress from localStorage using the same key as actividades.js
      const progreso = Number.parseInt(localStorage.getItem("progreso_actividades")) || 0
  
      // Update progress if this activity is the next one to complete
      if (activityNumber === progreso + 1) {
        ...
        // Call the global function if available (when returning to actividades.html)
        if (typeof window.completarActividad === "function") {
          window.completarActividad(activityNumber)
        }
      }
  
      // Also save individual completion status
